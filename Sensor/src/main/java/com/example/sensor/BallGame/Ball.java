...
public class Ball {
    ...
    private final double mAcceleration = 2.0 ;
    private final double mAttenuator = 0.02 ;
    private final double mBound = 0.8 ;

    public Ball (Drawable mDrawable, Rect mViewRect) {
    ...
    public void move (float mPitch, float mRoll)  {
        ...
        double dx = - Math.sin(Math.toRadians(mRoll)) *  mAcceleration ;
        double dy = - Math.sin(Math.toRadians(mPitch)) * mAcceleration ;
